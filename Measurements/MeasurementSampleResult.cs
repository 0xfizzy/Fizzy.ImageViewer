...
public sealed class MeasurementSampleResult : MeasurementResult
{
    public IReadOnlyList<PixelCoordinate> Coordinates { get; }
    public IReadOnlyList<PixelSample> Samples { get; }
    internal MeasurementSampleResult(Guid id, long version, FrameInfo frame, MeasurementQuery query,
        PixelCoordinate[] coordinates, PixelSample[] samples) : base(id, version, frame, query)
    {
        if (query is not (MeasurementQuery.Pixel or MeasurementQuery.LineProfile))
            throw new ArgumentOutOfRangeException(nameof(query));
        if (coordinates.Length != samples.Length || (query == MeasurementQuery.Pixel && samples.Length != 1))
